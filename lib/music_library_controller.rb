class MusicLibraryController
    ...
    def call
      ...
      def list_songs_by_artist
        ...
        userinput = gets.strip
        if artist = Artist.find_by_name(userinput)
      ...
      def list_songs_by_genre
        ...
        userinput = gets.strip
        if genre = Genre.find_by_name(userinput)
      ...
      def play_song
        ...
        if (1..Song.all.length).include?(userinput) #does song exist
            song = Song.all.sort{ |a, b| a.name <=> b.name }[userinput - 1]
        end
        if song != nil
